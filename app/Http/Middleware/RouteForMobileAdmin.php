...
class RouteForMobileAdmin
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if($user->isadmin == 0)
        {
            return response()->json(['result'=>false, 'message'=>'You have no admin permission!']);
        }

        if($user->approve == 0)
        {
            return response()->json(['result'=>false, 'message'=>'Your admin permission is not approved!']);
        }
